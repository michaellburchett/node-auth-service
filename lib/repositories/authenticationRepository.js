...
module.exports.createUserforAuthentication = async function(req, email, password, done) {
    var check_user = await (new User).fetchByField("email", email);
    if (check_user) return done(null, false, req.flash('errorMessage', errors.takenEmail ));//REturns but then continues returning
