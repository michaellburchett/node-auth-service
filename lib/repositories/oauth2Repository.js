...
module.exports.setup = function(server) {
    ...
    */
    server.grant(oauth2orize.grant.token(async (client, user, ares, done) => {
        generateAccessTokenAndRefreshToken(user.id, client.clientId, "token", (json) => {
    ...
    */
    server.exchange(oauth2orize.exchange.code(async (client, code, redirectUri, done) => {
        ...
        generateAccessTokenAndRefreshToken(authCode.user_id, authCode.client_id, "authorization", (json) => {
    ...
    server.exchange(oauth2orize.exchange.clientCredentials((client, scope, done) => {
        Client.getByClientId(client.clientId, function(clientmodel) {
            ...
            generateAccessTokenAndRefreshToken(null, client.clientId, "client", (json) => {
                return done(null, json);
            });
        });
    }));
    ...
    server.exchange(oauth2orize.exchange.password(async function (client, username, password, scope, done) {
        ...
        generateAccessTokenAndRefreshToken(user.id, client.clientId, "password", (json) => {
            return done(null, json);
        });
    }));
    ...
    */
    server.exchange(oauth2orize.exchange.refreshToken(async (client, refreshToken, done) => {
        ...
        generateAccessTokenAndRefreshToken(refreshToken.user_id, client.clientId, "refresh_token", (json) => {
            return done(null, json);
        });
    }));
 ...
 * @return {Object} information about the status of the request
 */
async function generateAccessTokenAndRefreshToken(user_id, client_id, type, done) {
